...
# Import an LLM client from LangChain / Google Gemini
...
class DBToolBase:
    def __init__(self, db_path: str, table_name: str, llm_model_name: str = "gemini-1.5-flash", unsafe_mode: bool = False):
        self.db_path = db_path
        self.table_name = table_name
        self.unsafe_mode = unsafe_mode
        # Use GeminiLLM — set GOOGLE_API_KEY in env
    ...
    def run(self, question: str, max_rows: int = 50) -> str:
        ...
        # Step 2: Validate (skip if unsafe mode is enabled)
        if not self.unsafe_mode and not self._validate_sql(sql):
